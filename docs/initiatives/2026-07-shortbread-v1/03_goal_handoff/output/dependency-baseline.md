...
|---|---|
| [`jdx/mise`](https://github.com/jdx/mise), [`jdx/aube`](https://github.com/jdx/aube), [`jdx/fnox`](https://github.com/jdx/fnox), [`jdx/hk`](https://github.com/jdx/hk), [`mise-plugins/mise-postgres`](https://github.com/mise-plugins/mise-postgres), [`rbenv/ruby-build`](https://github.com/rbenv/ruby-build) | MIT |
...
- **fnox residual advisory:** fnox `1.28.0`'s tagged dependency graph retains RustSec/OSV findings in inactive provider, authentication, TLS, and Keepass/XML paths; later `1.30.0` also hardens export/daemon secret handling. The frozen checkpoint invokes only offline `fnox ... scan`, has an empty provider configuration, and never invokes export, exec, daemon, network authentication, Keepass, or XML parsing. The controller therefore accepts this older binary as reachability-bounded and requires a fresh review before any fnox provider or runtime secret command is enabled.
- **Go telemetry isolation:** Go 1.23 and later [collect local toolchain counters by default](https://go.dev/doc/telemetry), and an independently opted-in user may upload an approved subset. The repository therefore sets the pinned Go 1.26.5 test-isolation hook `TEST_TELEMETRY_DIR` to checked-in `off` mode for every mise environment/task; the [pinned toolchain source](https://go.googlesource.com/go/+/refs/tags/go1.26.5/src/cmd/internal/telemetry/telemetry.go) passes that isolated directory to counters and upload startup. The dependency-policy gate requires the exact environment/path/mode and rejects any created counter/report file. This leaves the contributor's global Go telemetry preference untouched.
...
- **Tool artifact locking:** GoReleaser is pinned now, before T11 adds release configuration. AnyCable moved from mise's deprecated UBI backend to its GitHub backend; the repaired lock records seven target checksums over five distinct release assets and no provenance-attestation field. PostgreSQL retains the registry's asdf backend and its documented artifact-lock limitation, but its installer plugin is pinned to commit `dcbda94a5229b6906ecf87460584739d965d9ca0`; source-built Ruby pins ruby-build to commit `013c27d7e557b71b21bfa0f9c7af1081cf5411dc`. The public `bin/bootstrap` system-shell preflight rejects an already-cached PostgreSQL plugin with a different origin/revision or any local change before mise executes it. The ruby-build commit-addressed ZIP is transported over HTTPS but not separately checksum-verified by mise.
...
- **Go vulnerability audit:** current `govulncheck v1.6.0` imports Go telemetry, so it is not a committed tool or recurring task. The controller ran the exact scanner once from an ephemeral directory with telemetry startup disabled, records the reachable and module-level results in checkpoint evidence, and repeats that external audit at T13 and T16 through the repository's off-mode isolation. `go vet` remains static analysis and is not represented as vulnerability scanning.
