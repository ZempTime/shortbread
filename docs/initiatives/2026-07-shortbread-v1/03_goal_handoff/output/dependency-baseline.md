...
- **fnox residual advisory:** fnox `1.28.0` carries `quick-xml 0.40.1` through its Keepass provider, and later `1.30.0` also hardens export/daemon secret handling. The frozen checkpoint invokes only `fnox ... scan`, has no configured secret provider, and never invokes export, exec, daemon, Keepass, or XML parsing. The controller therefore accepts this older pin as reachability-bounded and requires a fresh review before any fnox provider or runtime secret command is enabled.
