...
- **fnox residual advisory:** fnox `1.28.0` carries `quick-xml 0.40.1` through its Keepass provider, and [fnox 1.30.0](https://github.com/jdx/fnox/releases/tag/v1.30.0) later hardened export and daemon secret handling. The frozen task graph invokes only `fnox ... scan`, has no configured provider or secrets, and never invokes export, exec, daemon, Keepass, or XML parsing. The controller accepts the residual for this checkpoint and requires reaudit before those fnox capabilities are enabled.
