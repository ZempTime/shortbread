...
- `mise.lock` contains 54 checksum-bearing platform records and 20 GitHub-attestation records. The PostgreSQL asdf backend emits no artifact record and locked mode is intentionally not claimed as universal; its executable plugin is separately pinned and checked at commit `dcbda94a5229b6906ecf87460584739d965d9ca0`. Ruby's source-build helper is pinned to ruby-build commit `013c27d7e557b71bfa0f9c7af1081cf5411dc`.
...
- **Package-manager/tool repair:** mise now requires at least `2026.7.1`, the fixed release for [GHSA-9mm4-fgvc-x7rp](https://github.com/jdx/mise/security/advisories/GHSA-9mm4-fgvc-x7rp); the installed and audited controller copy is `2026.7.7`. The checked-in setting `use_versions_host_track = false` overrides mise's documented default, which would [send anonymous tool/version and host install statistics](https://mise.jdx.dev/configuration/settings.html#use-versions-host-track) after successful installs. Aube moved from `1.25.2` to `1.29.1`, whose [release](https://github.com/jdx/aube/releases/tag/v1.29.1) fixes the security-relevant `minimumReleaseAge` behavior. AnyCable Go `1.6.15` is the fixed line for [GHSA-w72w-9qmj-c9qm](https://github.com/anycable/anycable/security/advisories/GHSA-w72w-9qmj-c9qm) and [GHSA-5p54-whvp-x327](https://github.com/anycable/anycable/security/advisories/GHSA-5p54-whvp-x327); the repo environment sets `ANYCABLE_DISABLE_TELEMETRY=true`, following the tagged [AnyCable opt-out documentation](https://github.com/anycable/anycable/blob/v1.6.15/docs/telemetry.md), so its anonymous PostHog-backed usage reporting is disabled.
...
- **fnox residual advisory:** fnox `1.28.0`'s tagged dependency graph retains RustSec/OSV findings in inactive provider, authentication, TLS, and Keepass/XML paths; [fnox 1.30.0](https://github.com/jdx/fnox/releases/tag/v1.30.0) also hardened export and daemon secret handling. The frozen task graph invokes only offline `fnox ... scan`, has an empty provider configuration, and never invokes export, exec, daemon, network authentication, Keepass, or XML parsing. The controller accepts this residual for the checkpoint and requires reaudit before those fnox capabilities are enabled.
- **Go telemetry isolation:** [Go telemetry documentation](https://go.dev/doc/telemetry) confirms that Go 1.23+ defaults to local counter collection and may upload when a user has independently opted in. Every checked-in mise environment/task now points the pinned Go 1.26.5 `TEST_TELEMETRY_DIR` hook to `config/go-telemetry`, whose only file is exact `off` mode; the [pinned Go source](https://go.googlesource.com/go/+/refs/tags/go1.26.5/src/cmd/internal/telemetry/telemetry.go) uses that hook for counters and upload startup. The policy gate requires the exact path/mode before setup and rejects any additional file afterward. A before/after SHA-256 snapshot proved that `go version`, `gofmt`, tests, and builds left the user's global telemetry directory unchanged while the repository isolation directory remained mode-only.
...
| Ruby | `3.4.10` | [Ruby OR BSD-2-Clause](https://github.com/ruby/ruby/blob/v3_4_10/COPYING) | 7 target URL/checksum entries: 6 reuse the ruby-lang source archive and Windows uses RubyInstaller; ruby-build is commit-pinned; no provenance field. |
...
| PostgreSQL | `17.10` | [PostgreSQL License](https://github.com/postgres/postgres/blob/REL_17_10/COPYRIGHT) | Exact `asdf:postgres` version and commit-pinned installer plugin; the backend does not emit an artifact URL, checksum, or provenance record. |
...
The PostgreSQL installer plugin and ruby-build are both MIT-licensed. The reviewed plugin is commit `dcbda94a5229b6906ecf87460584739d965d9ca0`; the public system-shell bootstrap rejects a cached origin, revision, or dirty tree mismatch before mise can execute it. Ruby-build is commit `013c27d7e557b71bfa0f9c7af1081cf5411dc`; its commit-addressed GitHub ZIP is HTTPS-fetched but not separately checksum-verified by mise.
